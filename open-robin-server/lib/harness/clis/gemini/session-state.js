...
/**
 * Session state for a single Gemini ACP thread.
...
class GeminiSessionState {
  constructor() {
    ...
    /** @type {boolean} — true once the first user prompt has been sent */
    ...
    /** @type {Array<{model: string, input_tokens: number, output_tokens: number}> | null} */
    this.modelUsage = null;
    
    // Context usage - always null for Gemini (cannot reliably calculate due to sub-agent black box)
    /** @type {number | null} */
   ...
   * @param {Object} quota
   * @param {number} quota.input_tokens
   * @param {number} quota.output_tokens
   * @param {Array} [quota.model_usage]
   */
  setTokenUsage(quota) {
    if (quota) {
      this.inputTokens = quota.input_tokens || null;
      this.outputTokens = quota.output_tokens || null;
      this.modelUsage = quota.model_usage || null;
    }
  }
...
module.exports = { GeminiSessionState };
