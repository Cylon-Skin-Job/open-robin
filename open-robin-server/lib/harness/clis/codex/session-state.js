...
/**
 * Session state for a single Codex ACP thread.
...
class CodexSessionState {
  ...
  startToolCall(toolCallId, toolName, title, rawInput) {
    ...
    if (rawInput) {
      try {
        ...
        // Fallback for partial JSON or non-JSON input
        args = { raw: rawInput };
      }
    }
  ...
  /**
   * Set token usage from prompt response or update
   * @param {Object} usage
  ...
  setTokenUsage(usage) {
    if (usage) {
      ...
      // You could also track totalTokens or others if needed
    }
  }
...
module.exports = { CodexSessionState };
